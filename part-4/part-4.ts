  ...
  async getLowestPrice(timeout: number, providers: any) {
    ...
    await Promise.all(
      providers?.map(async (provider: any) => {
        ...
        let priceObj = await providerResult.getPrice();

        if (provider.timeout < timeout) {
          if (priceObj) priceArray.push(priceObj);
        }
      })
    );

    if (priceArray.length) {
      result = priceArray?.reduce((prevPrice, currPrice) =>
        prevPrice.price < currPrice.price ? prevPrice : currPrice
      );
    } else {
      result = null;
    }
